...
namespace BL
{
    public class Direccion
    {
        ...
        public Direccion(DL.AreyesDiciembreContext context)
        {
            _context = context;
        }
        public ML.Result Add(ML.Usuario usuario)
        {
            ...
            try
            {
                ...
                int filasAfectadas = _context.Database.ExecuteSqlRaw("EXEC DireccionADD @Calle, @NumeroExterior, @NumeroInterior, @IdColonis, @IdUsuario", parametros);
        ...
        public ML.Result Update(ML.Usuario usuario)
        {
            ...
            try
            {
                ...
                int filasAfectadas = _context.Database.ExecuteSqlRaw("EXEC DireccionUpdate @IdDireccion, @Calle, @NumeroExterior, @NumeroInterior, @IdColonia", parametros);
        ...
        public ML.Result GetByIdDireccion(int IdDireccion)
        {
            ML.Result result = new ML.Result();
            try
            {
                var direccion = _context.DireccionGetByIdDTO.FromSqlRaw($"EXEC DireccionGetById {IdDireccion}").AsEnumerable().SingleOrDefault();
                if (direccion != null)
                {
        ...
        public ML.Result Delete(int IdDireccion)
        {
            ML.Result result = new ML.Result();
            try
            {
               int filasAfectadas = _context.Database.ExecuteSql($"EXEC DireccionDelete @IdDireccion={IdDireccion}");
                    if (filasAfectadas > 0)
                    {
                        result.Correct = true;
                    }
